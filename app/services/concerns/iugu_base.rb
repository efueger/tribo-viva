module IuguBase
  ...
  def save_purchase
    purchase.attributes = { taxes: taxes, invoice_id: invoice.id, invoice_url: invoice.secure_url, invoice_pdf: charge.pdf }
    purchase.save
  end
  ...
  def redirect_path
    if success?
      ...
      [Rails.application.routes.url_helpers.checkout_path, flash: { charge_messages: charge.message || 'Verifique os dados digitados' } ]
    end
  end
end
